...
const ItemCreate = (props) => {
    ...
    const toggleTwitter = () => setIsTwitter(!isTwitter)

    const[tweetURL, setTweetURL] = useState("")
    const handleTweetURL = e => {
    ...
    const submitItem = (e) => {
        const parsedTweet = tweetURL.substring(tweetURL.lastIndexOf('/') + 1)
        UploadToAws(selectedFile, url => {
            ...
            if (!props.isBounty) {
                ...
                data.award = cost;
                data.tweetId = isTwitter ? parsedTweet : -1;
                data.manual = !isTwitter;
            }
        ...
        <Container fluid="sm">
            ...
            {props.isBounty &&
                <Row style={{ "justifyContent": "center", "textAlign": "left" }}>
                    <Col md="auto" style={{ "paddingTop": "8px" }}>
                        <FormControlLabel
                            control={<Switch onChange={toggleTwitter} />}
                            label="Twitter Trigger"
                        />
                    </Col>
                    <Col md={3}>
                       {isTwitter && <TextField label="Tweet URL" variant="outlined" margin="dense" fullWidth shrink="true"
                            onChange={handleTweetURL}
                            value={tweetURL}
                        required/> }
                    </Col>
                </Row>}
